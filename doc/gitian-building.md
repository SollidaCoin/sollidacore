...
*Setup instructions for a Gitian build of sollida Core using a Debian VM or physical system.*
...
- [Building sollida Core](#building-sollida-core)
...
Clone the git repositories for sollida and Gitian.
...
in the sollida repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building sollida Core
----------------

To build sollida Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sollida repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/sollida-project/sollida
...
For example:
```bash
URL=https://github.com/thrasher-/sollida.git
...
and the sollida git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sollida/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sollida
SIGPATH=/some/root/path/sollida-detached-sigs

./bin/gbuild --url sollida=${BTCPATH},signature=${SIGPATH} ../sollida/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sollida-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sollida-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sollida-osx-build.assert
```
...
[sollida-project/gitian.sigs.ltc](https://github.com/sollida-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
